...
GRO coalesces TCP data segments on ingress — it does not coalesce ICMP. PTB packets
(ICMP type 3 code 4) are not subject to GRO, so the PTB counter at TC ingress is
expected to be unaffected by GRO. **This has not been validated under all offload
combinations** — see the integration test section below.
...
**`VXLAN_MTU_RISK`** is potentially unreliable when GSO is enabled on the overlay
interface. A large `max_outer_ip_len` with no `ip_do_fragment` events may be explained
by GSO rather than an actual MTU problem. The verdict message should be treated as a
hypothesis, not a confirmation.

**`VXLAN_FRAGMENTATION_OBSERVED`** via `ip_do_fragment` is expected to be unaffected by
GSO: `ip_do_fragment` is called when the IP layer actually fragments a packet, and GSO
segmentation typically happens before `ip_do_fragment` is reached. However, the
interaction between GSO, the TC size corroboration signal, and `ip_do_fragment` under
all offload combinations has **not been validated with live tests**. Do not claim
`VXLAN_FRAGMENTATION_OBSERVED` is "definitely unaffected by GSO" until
`evidence/rc2-gso-gro-live.md` contains real test results.

**`PTB_SUPPRESSED` and `PTB_DELIVERED`** via TC ingress and `icmp_rcv` are expected to
be unaffected by GSO/GRO, but this also has not been validated with live offload tests.
...
|---------|-----------------|------------------------|
| `VXLAN_FRAGMENTATION_OBSERVED` | Likely not (expected) | **UNVALIDATED** — not tested under all offload combos |
| `PTB_SUPPRESSED` | Likely not (expected) | **UNVALIDATED** — not tested with GRO on/off |
| `PTB_DELIVERED` | Likely not (expected) | **UNVALIDATED** |
...
| `NO_ISSUE_OBSERVED` | Indirectly | May miss VXLAN_MTU_RISK if GSO hides it |
...
Required live tests before any GSO/GRO verdict status can be changed from UNVALIDATED:

1. **GSO on, TCP traffic:** Run `iperf3` over the VXLAN overlay, observe `max_outer_ip_len`
   at TC egress, compare with actual wire captures from `tcpdump`. Record: is
   `max_outer_ip_len` the GSO super-packet size or the wire size?

2. **GSO off, TCP traffic:** Same test with `ethtool -K <overlay_iface> tx off`. Record
   whether `max_outer_ip_len` changes to match wire captures.

3. **GRO on/off, PTB injection:** Inject ICMP PTBs with `scripts/inject_ptb.py`, observe
   TC ingress count with GRO on and off. Record: does GRO affect the PTB count?

4. **ip_do_fragment with GSO on:** Set up a fragmentation scenario (overlay MTU > underlay
   MTU, DF=0). Observe whether `frag_events_total` fires correctly regardless of GSO state.

Results must be saved to: `evidence/rc2-gso-gro-live.md` (currently NOT RUN).
